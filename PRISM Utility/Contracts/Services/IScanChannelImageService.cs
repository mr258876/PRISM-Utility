...
public interface IScanChannelImageService
{
    ...
    Task SaveRgbImageAsync(StorageFile file, ScanCompositeFrame frame, CancellationToken cancellationToken = default);
    Task SaveRgbImageAsync(StorageFile file, ScanCompositePixelBuffer buffer, CancellationToken cancellationToken = default);
    Task<StorageFolder?> PickDngExportFolderAsync();
    Task<ScanDngExportResult> ExportDngChannelsAsync(StorageFolder folder, ScanWorkflowResult result, ScanChannelAssignment assignment, ScanChannelAlignmentMode alignmentMode, ScanDngExportMode exportMode = ScanDngExportMode.LinearRaw4, IReadOnlyDictionary<string, ScanChannelCalibrationProfile>? channelProfiles = null, CancellationToken cancellationToken = default);
    Task ExportMonochromeDngAsync(StorageFolder folder, byte[] pixelData, int rows, ushort exposureTicks, uint sysClockKhz, string channelLabel, ScanChannelCalibrationProfile? profile = null, CancellationToken cancellationToken = default);
