...
| --- | --- | --- | --- | --- |
| [CAL-001](issues-and-remediation.md) | 已关闭测试缺口 | managed branch 已有 15 个 deterministic fake tests 覆盖黑场和白场收敛、黑白振荡最佳采样回退、invalid width/ROI clamp、scan failure、motion timeout、cancellation、warm-up cleanup、0/2 号焦点电机 all-path stop 和未归一化 Brenner 锁定。 | 不把 managed fake harness 结果写成物理 hardware smoke；CAL-002 保持独立未关闭。 | CAL001 focused tests 15/15、full managed suite 555/555、Core/app builds 和 Full validator 通过；真实 scanner smoke 仍为 `ENVIRONMENT_BLOCKED`。 |
