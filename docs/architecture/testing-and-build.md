...
测试项目 `Host Software/PrismUtility.Core.Tests/PrismUtility.Core.Tests.csproj` 目标框架是 `net8.0-windows10.0.19041.0`，启用 nullable 和 preview 语言版本，引用 xUnit、Microsoft.NET.Test.Sdk、coverlet，并链接当前 source-contract tests 所需的导航、设置、本地化、PageService 和 helper 源。胶片配置 fixture 会从 Fixtures\FilmProfile 下的 JSON 文件复制到输出目录。Schema v6 是当前写入基线；v5 文档仍按兼容迁移路径读取，并由 schema regression 覆盖。
...
| Film profile services | `Host Software/PrismUtility.Core.Tests/ScanFilmProfileDocumentServiceTests.cs`, `Host Software/PrismUtility.Core.Tests/ScanFilmProfileWorkspaceTests.cs`, `Host Software/PrismUtility.Core.Tests/ScanCalibrationProfileRepositoryTests.cs` | Schema v6 parsing/writing, v5 migration, malformed and future documents, workspace dirty state, import staging, repository migration and write failures. | Hardware produced calibration values and full UI file picker flow. |
| Todo 15 calibration status | `Host Software/PrismUtility.Core.Tests/ScanDebugCalibrationStatusTests.cs`, `Host Software/PrismUtility.Core.Tests/Cal001AutoCalibrationAndFocusTests.cs` | Latest available software-gate snapshot reports calibration status 104/104, covering pending review candidate behavior, Accept / Accept and Save / Restore / Cancel state contracts, warm-up-settled before/after metrics and CAL-001 fake service algorithm coverage. | Physical scanner, real illumination, restoration on device, calibration quality, real focus motion and hardware smoke remain `ENVIRONMENT_BLOCKED` or `NEEDS_DEVICE_VALIDATION`. |
| Scan workflow and sessions | `Host Software/PrismUtility.Core.Tests/ScanWorkflowServiceTests.cs`, `Host Software/PrismUtility.Core.Tests/ScanDebugCalibrationStatusTests.cs`, `Host Software/PrismUtility.Core.Tests/ScanWorkflowSessionCoordinatorTests.cs`, `Host Software/PrismUtility.Core.Tests/ScannerLifecycleIntegrationRegressionTests.cs` | Delta row snapshots, FIFO delivery, callback isolation, preview generation lifecycle, contiguous/stale/gap/legacy preview merges, illumination off before returns, motor transport disabled, progress metadata, warm-up ownership and cleanup. | Live scanner timing, physical warm-up behavior and hung motion behavior on real firmware. |
...
| Todo 15 calibration software snapshot | Record the latest available managed calibration gate without claiming hardware validation. | Calibration status 104/104, parent focused gate 183/183, full managed suite 1034/1034, Core build clean, and x64 app build clean except the pre-existing WindowsAppSDK SingleFile warning. | This is a snapshot, not a new hardware/native claim. Physical scanner, illumination, restoration on device, calibration quality and hardware smoke remain `ENVIRONMENT_BLOCKED` or `NEEDS_DEVICE_VALIDATION`, and CAL-002 remains unchanged. |
